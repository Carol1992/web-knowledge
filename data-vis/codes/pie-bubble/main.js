...
export default class Table {
  ...
  draw () {
    ...
    //增加子分组，画标签
    let tableContainer = main.append('g').attr('class', type)
    tableContainer.attr('transform', `translate(${margin_left}, ${margin_top})`)

    all_data.forEach(data => {
      let pieContainer = tableContainer.append('g')
        .attr('class', `${type}-${data.x}-${data.y}`)
        .attr('transform', `translate(${data.x * config.table_body_width}, ${data.y * config.table_body_height})`)
      if(type === 'pie') {
        drawPie({data: data.values, container: pieContainer, config})
      } else {
        drawBubble({data: data.values, container: pieContainer, config})
      }
    })
  }
}